...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (433083,4084011,4156145)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (433083,4084011,4156145)
...
WHERE (C.condition_start_date >= DATEFROMPARTS(2006, 01, 01) and C.condition_start_date <= DATEFROMPARTS(2015, 12, 31))
...
WHERE DATEADD(day,90,OP.OBSERVATION_PERIOD_START_DATE) <= P.START_DATE AND DATEADD(day,90,P.START_DATE) <= OP.OBSERVATION_PERIOD_END_DATE
...
(
  ...
  LEFT JOIN
  (
    select ET.person_id, ET.event_id from PrimaryCriteriaEvents ET
