WITH rx_data AS (
	...
	WHERE concept_ancestor.ancestor_concept_id = 21604303 
	AND @target_cohort_table.cohort_start_date - drug_exposure.drug_exposure_start_date <= 365
	AND @target_cohort_table.cohort_start_date - drug_exposure.drug_exposure_start_date > 180
)

SELECT AVG(A.avg_pp_quantity) as average_quantity
FROM (
	SELECT rx_data.subject_id, AVG(rx_data.quantity) AS avg_pp_quantity 
	FROM rx_data
