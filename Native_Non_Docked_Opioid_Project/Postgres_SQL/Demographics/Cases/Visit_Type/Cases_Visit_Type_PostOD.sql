...
FROM(
	SELECT A.subject_id, COUNT(A.visit_occurrence_id)/185.0 AS per_day_per_person_avg
		...
		WHERE @target_cohort_table.cohort_start_date - visit_occurrence.visit_end_date  >= -180
		AND @target_cohort_table.cohort_start_date - visit_occurrence.visit_start_date < 0
