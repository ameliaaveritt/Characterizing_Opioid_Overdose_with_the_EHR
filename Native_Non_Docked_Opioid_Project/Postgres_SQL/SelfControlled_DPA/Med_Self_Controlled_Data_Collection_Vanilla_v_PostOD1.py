...
SELECT DISTINCT B.drug_concept_id FROM (
	SELECT opioid_cases.subject_id, opioid_cases.cohort_start_date, drug_exposure.drug_concept_id, concept.concept_name, 
	opioid_cases.cohort_start_date, drug_exposure.drug_exposure_start_date, opioid_cases.cohort_start_date - drug_exposure.drug_exposure_start_date
	FROM opioid_cases	
	LEFT JOIN public.drug_exposure ON (opioid_cases.subject_id = drug_exposure.person_id) 
	JOIN public.concept ON (drug_exposure.drug_concept_id = concept.concept_id) 
	JOIN public.concept_ancestor ON (concept.concept_id=concept_ancestor.ancestor_concept_id)
	WHERE opioid_cases.subject_id = {}
	AND concept.concept_class_id = 'Ingredient'
	AND opioid_cases.cohort_start_date - drug_exposure.drug_exposure_start_date > 180
	AND opioid_cases.cohort_start_date - drug_exposure.drug_exposure_start_date <= 365
...
print "PreOD1 Period -6m thru 0m"

for x in ids_OD_pos:
	# print i
	cursor2=db.cursor()
	query2 = """SELECT DISTINCT B.drug_concept_id FROM (
	SELECT opioid_cases.subject_id, opioid_cases.cohort_start_date, drug_exposure.drug_concept_id, concept.concept_name, 
	opioid_cases.cohort_start_date, drug_exposure.drug_exposure_start_date, opioid_cases.cohort_start_date - drug_exposure.drug_exposure_start_date
	FROM opioid_cases	
	LEFT JOIN public.drug_exposure ON (opioid_cases.subject_id = drug_exposure.person_id) 
	...
	WHERE opioid_cases.subject_id = {}
	AND concept.concept_class_id = 'Ingredient'
	AND opioid_cases.cohort_start_date - drug_exposure.drug_exposure_start_date > 0
	AND opioid_cases.cohort_start_date - drug_exposure.drug_exposure_start_date <= 180
	AND drug_exposure.drug_concept_id <> '0'
) B ;	;""".format(x)
	
	cursor2.execute(query2)
	concepts_PreOD1 = cursor2.fetchall()
	concepts_PreOD1 = [str(b[0]) for b in concepts_PreOD1]
	uniq_concepts.update(concepts_PreOD1)

	PreOD1_dict[x]=concepts_PreOD1


print "Getting 2x2 Data"

with open ("output/Meds_Vanilla_v_PreOD1.csv", "w") as file:
	...
	for concept in uniq_concepts:
		...
		for person in ids_OD_pos:
			#POST = POSTOD1 PERIOD	
			if concept in PreOD1_dict[person]:
				Post_Pos += 1.0
			else:
				Post_Neg +=1.0
			#PRE = VANILLA PERIOD
